...
class OpenWeatherDataRetrieverTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @concern_user = ConcernUser.new

    # stub for returning data in successful scenarios
    def @concern_user.open_weather_client(api_key)
      client = Object.new
      def client.one_call(opts)
  ...
  test "retrieve weather (fahrenheit)" do
    data, success = @concern_user.retrieve_weather_data(@request_opts)
    assert_equal [@expected_fahrenheit_data, true], [data, success]
  end

  test "retrieve weather (celsius)" do
    data, success = @concern_user.retrieve_weather_data(@request_opts.merge(temp_unit: "celsius"))
  ...
  test "should return cached result on second request" do
    ...
    data, success = @concern_user.retrieve_weather_data(@request_opts)
    assert_equal [@expected_fahrenheit_data.merge(retrieved_from_cache: true), true], [data, success]
  end

  test "should fail gracefully when required field is missing" do
    data, success = @concern_user.retrieve_weather_data(@request_opts.except(:latitude))
  ...
  test "should fail gracefully when OpenWeather cannot return data" do
    ...
    def @concern_user.download_raw_data(opts)
      [{}, false]
    end

    data, success = @concern_user.retrieve_weather_data(latitude: 200, longitude: 200, city: "Chula Vista", state: "CA", zipcode: "91913", country: "US", temp_unit: "fahrenheit")
    assert_equal [{}, false], [data, success]
  end
end
